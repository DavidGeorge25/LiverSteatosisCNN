...
from .detect import MACRO, MICRO, FatResult, detect_fat
from ...core.io import SlideOutputs
from ...core.slide import SLIDE_EXTENSIONS, Slide, find_slides
...
from .viz import tile_qc_figure
...
def run_slide(cfg: Config, verbose_tiles: int = 20, detail: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        out = SlideOutputs.for_slide(out_root, slide.name).mkdirs(FEATURE)
        ...
        for i, tile in enumerate(iter_tiles(slide, tissue, cfg.tiling, limit=cfg.limit)):
            ...
            if cfg.tiling.save_tiles:
                out.write_tile(rgb, tile.name)
                out.write_mask(res.label_mask, FEATURE, f"{tile.name}_mask")
            ...
            if i in sample_idx:
                ...
                if cfg.qc.save_per_tile_qc:
                    save_rgb(
                        out.feature_qc_dir(FEATURE) / "tiles" / f"{tile.name}_qc.png",
                        fig,
                    )
        ...
        csv_path = out.write_table(df, FEATURE, "tiles")
        ...
        if sheet_panels:
            ...
            sheet_path = out.write_qc(sheet, FEATURE, "contact_sheet")
...
def _batch_worker(args) -> dict[str, Any]:
    ...
    stem = Path(path).stem
    log_path = SlideOutputs.for_slide(slide_cfg.output_dir, stem).log_path()
    try:
